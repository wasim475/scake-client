...
import { Input, Select } from '../../utility/helper';
import axios from 'axios';
import { toast } from 'react-toastify';

const CATEGORIES = ["Birthday", "Wedding", "Chocolate", "Fruit", "Custom", "Cupcakes", "Theme", "Cheesecake"];

 const EMPTY_FORM = { name: "", emoji: "🎂", category: "Birthday", price: "", description: "", stock: true, imageLink:"" };




const AddCakeModal = ({ onSave, onClose }) => {
  ...
  const imageInfo = new FormData();
    imageInfo.append("image", file);

  const res = await axios.post(`http://localhost:3000/api/v1/cakes/create-image-link`,imageInfo,
     {
    headers: {
...
// console.log(form)
  ...
  const handleSave = async() => {
    ...
    const res = await axios.post(`http://localhost:3000/api/v1/cakes/create-cake`,form)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
          ...
          {/* Image upload */}
          <div
            onClick={() => document.getElementById("cakeImageInput").click()}
            className="border-2 border-dashed border-gray-200 rounded-xl p-6 flex flex-col items-center gap-2 hover:border-[#E8627A]/40 transition-colors cursor-pointer"
          >
            <input
              id="cakeImageInput"
              type="file"
  ...
  <div className="relative">
    ...
    <button
      onClick={(e) => {
        e.stopPropagation(); // upload click prevent
        // handleRemoveImage();
        setForm((f) => ({
            ...f,
            image: null,
    ...
    >
      <X size={12} />
        ...
        <div className="sticky bottom-0 bg-white px-6 py-4 border-t border-gray-100 flex gap-3 rounded-b-2xl">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 text-sm font-semibold text-[#8B7070] border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors"
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            className="flex-1 py-2.5 text-sm font-bold text-white bg-[#E8627A] hover:bg-[#C04060] rounded-xl transition-colors active:scale-95"
          >
            Add Cake
          </button>
        </div>
